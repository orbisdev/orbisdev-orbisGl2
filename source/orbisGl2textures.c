...
{
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s \n",__FUNCTION__);
...
void orbisGlDestroyImage(Image *img)
{
	...
	if(img!=NULL)
	{
		debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s width=%d height=%d format=%d \n",__FUNCTION__,img->width,img->height,img->format);
		...
		//	munmap(img->data,orbisGlGetPixelDataSize(img->width,img->height,img->format));
			free(img->data);
			img->data=NULL;
			debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s after free data \n",__FUNCTION__);

		}
		//free(img);
		debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s image destroyed\n",__FUNCTION__);
...
static void orbisGlReadPngFromBuffer(png_structp png_ptr, png_bytep data, png_size_t length)
{
	...
	*address += length;
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s end\n",__FUNCTION__);
...
static Image orbisGlLoadPngGeneric(const void *io_ptr,png_rw_ptr read_data_fn)
{
	...
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s after png_create_read_struct \n",__FUNCTION__);
	...
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s after png_create_info_struct \n",__FUNCTION__);

	png_bytep *row_ptrs=NULL;

	if (setjmp(png_jmpbuf(png_ptr))) 
	{
		debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s after setjmp wrong \n",__FUNCTION__);
	...
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s after setjmp right \n",__FUNCTION__);

	png_set_read_fn(png_ptr,(png_voidp)io_ptr,read_data_fn);
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s after png_set_read_fn \n",__FUNCTION__);
	png_set_sig_bytes(png_ptr,PNG_SIGSIZE);
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s after png_set_sig_bytes \n",__FUNCTION__);
	png_read_info(png_ptr,info_ptr);
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s after png_read_info \n",__FUNCTION__);
	...
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s after png_get_IHDR \n",__FUNCTION__);
		...
		|| (bit_depth==16))
	{
		...
		debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s after png_set_expand \n",__FUNCTION__);

	}
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s bitdepth=%d end\n",__FUNCTION__,bit_depth);
	if (bit_depth == 16)
	...
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s png_read_image end\n",__FUNCTION__);

	png_destroy_read_struct(&png_ptr, &info_ptr, (png_infopp)0);
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s before free\n",__FUNCTION__);
...
error_create_tex:
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s error crete_text\n",__FUNCTION__);
	free(row_ptrs);
error_alloc_rows:
	png_destroy_info_struct(png_ptr,&info_ptr);
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s error destroy_info_struct\n",__FUNCTION__);

error_create_info:
	png_destroy_read_struct(&png_ptr,(png_infopp)0,(png_infopp)0);
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s error create_info\n",__FUNCTION__);
error_create_read:
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s error_create_read\n",__FUNCTION__);
...
Image LoadImageRaw(const char *fileName, int width, int height, int format, int headerSize)
{
	...
	if (rawFile < 0)
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s %s  RAW image file could not be opened\n",__FUNCTION__,fileName);
	}
	else
	{
		...
		if (bytes < size)
		{
			debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s %s RAW image data can not be read, wrong requested format or size\n",__FUNCTION__,fileName);
...
Texture2D LoadTextureFromImage(Image image)
{
	...
	else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Texture could not be loaded from Image\n",__FUNCTION__);
...
void UnloadTexture(Texture2D texture)
{
    if (texture.id > 0)
    {
        ...
        debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s TEX ID=%i Unloaded texture data from VRAM (GPU)\n",__FUNCTION__,texture.id);
    }
}
...
Color *GetImageData(Image image)
{
	...
	if(pixels==NULL)
	{
		debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] %s error in malloc\n",__FUNCTION__);
		return NULL;
	}
	if (image.format >= COMPRESSED_DXT1_RGB) debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Pixel data retrieval not supported for compressed image formats\n",__FUNCTION__);
	else
	{
		if ((image.format == UNCOMPRESSED_R32) ||
		    (image.format == UNCOMPRESSED_R32G32B32) ||
		    (image.format == UNCOMPRESSED_R32G32B32A32)) debugNetPrintf(DEBUGNET_DEBUG, "[ORBISGL] %s 32bit pixel format converted to 8bit per channel\n",__FUNCTION__);
...
Image GetTextureData(Texture2D texture)
{
	...
	if (texture.format < 8)
	{
		...
		if (image.data != NULL)
		{
			...
			image.format = UNCOMPRESSED_R8G8B8A8;
			debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s Texture pixel data obtained successfully\n",__FUNCTION__);
		}
		else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Texture pixel data could not be obtained\n",__FUNCTION__);
	}
	else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Compressed texture data could not be obtained\n",__FUNCTION__);
...
void ImageToPOT(Image *image, Color fillColor)
{
	...
	if ((potWidth != image->width) || (potHeight != image->height))
	{
		...
		debugNetPrintf(DEBUGNET_DEBUG, "[ORBISGL] %s Image converted to POT: (%ix%i) -> (%ix%i)",__FUNCTION__, image->width, image->height, potWidth, potHeight);
...
void ImageFormat(Image *image, int newFormat)
{
    ...
    if ((newFormat != 0) && (image->format != newFormat))
    {
        ...
        else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Image data format is compressed, can not be converted\n",__FUNCTION__);
    }
}
...
void ImageAlphaMask(Image *image, Image alphaMask)
{
    ...
    {
        debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Alpha mask must be same size as image\n",__FUNCTION__);
    }
    else if (image->format >= COMPRESSED_DXT1_RGB)
    {
        debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Alpha mask can not be applied to compressed data formats\n",__FUNCTION__);
    }
    else
    {
...
TextureCubemap LoadTextureCubemapFromImage(Image image, int layoutType)
{
	...
	if (layoutType != CUBEMAP_AUTO_DETECT)
	{
		...
		if (cubemap.id == 0) debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Cubemap image could not be loaded.\n",__FUNCTION__);
		UnloadImage(faces);
	}
	else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Cubemap image layout can not be detected.\n",__FUNCTION__);
	return cubemap;
}
...
void ImageCrop(Image *image, Rectangle crop)
{
	...
	else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Image can not be cropped, crop rectangle out of bounds \n",__FUNCTION__);
}
...
void ImageMipmaps(Image *image)
{
	...
	while ((mipWidth != 1) || (mipHeight != 1))
	{
		...
		debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s Next mipmap level: %i x %i - current size %i\n",__FUNCTION__,mipWidth, mipHeight, mipSize);
		mipCount++;
		mipSize += GetPixelDataSize(mipWidth, mipHeight, image->format);       // Add mipmap size (in bytes)
	}
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s Mipmaps available: %i - Mipmaps required: %i\n",__FUNCTION__,image->mipmaps, mipCount);
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s Mipmaps total size required: %i\n",__FUNCTION__,mipSize);
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s Image data memory start address: 0x%x\n",__FUNCTION__,image->data);
	if (image->mipmaps < mipCount)
	{
		...
		if (temp != NULL)
		{
			...
			debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s Image data memory point reallocated: 0x%x\n",__FUNCTION__, temp);
		}
		else debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s Mipmaps required memory could not be allocated\n",__FUNCTION__);
		...
		{
			debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s Gen mipmap level: %i (%i x %i) - size: %i - offset: 0x%x\n",__FUNCTION__, i, mipWidth, mipHeight, mipSize, nextmip);
	...
	else debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s Image mipmaps already available\n",__FUNCTION__);
}
...
void ImageDither(Image *image, int rBpp, int gBpp, int bBpp, int aBpp)
{
	...
	{
		debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] %s Compressed data formats can not be dithered\n",__FUNCTION__);
		return;
	}
	if ((rBpp + gBpp + bBpp + aBpp) > 16)
	{
		debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] %s Unsupported dithering bpps (%ibpp), only 16bpp or lower modes supported\n",__FUNCTION__, (rBpp+gBpp+bBpp+aBpp));
	}
	else
	{
		Color *pixels = GetImageData(*image);
		...
		{
			debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] %sImage format is already 16bpp or lower, dithering could have no effect\n",__FUNCTION__);
		}
		...
		{
			image->format = 0;
			debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] %s Unsupported dithered OpenGL internal format: %ibpp (R%iG%iB%iA%i)\n",__FUNCTION__, (rBpp+gBpp+bBpp+aBpp), rBpp, gBpp, bBpp, aBpp);
		}
		...
		if (pixels[i].a > 0)
		{
			...
			if (!colorInPalette)
			{
				...
				if (palCount >= maxPaletteSize)
				{
					...
					debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Image palette is greater than %i colors!\n",__FUNCTION__, maxPaletteSize);
				}
			}
		}
	}
...
void ImageDraw(Image *dst, Image src, Rectangle srcRec, Rectangle dstRec, Color tint)
{
	...
	if ((srcRec.x + srcRec.width) > src.width)
	{
		...
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Source rectangle width out of bounds, rescaled width: %i\n",__FUNCTION__, srcRec.width);
	}
	if ((srcRec.y + srcRec.height) > src.height)
	{
		srcRec.height = src.height - srcRec.y;
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Source rectangle height out of bounds, rescaled height: %i\n",__FUNCTION__, srcRec.height);
	}
...
Image ImageTextEx(Font font, const char *text, float fontSize, float spacing, Color tint)
{
	...
	if (fontSize > imSize.y)
	{
		...
		debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s Image text scaled by factor: %f\n",__FUNCTION__,scaleFactor);
...
void SetTextureFilter(Texture2D texture, int filterMode)
{
	switch (filterMode)
	{
		...
		case FILTER_TRILINEAR:
		{
			...
			{
				debugNetPrintf(DEBUGNET_DEBUG, "[ORBISGL] %s TEX ID %i No mipmaps available for TRILINEAR texture filtering",__FUNCTION__, texture.id);
				// RL_FILTER_LINEAR - tex filter: BILINEAR, no mipmaps
				rlTextureParameters(texture.id, RL_TEXTURE_MIN_FILTER, RL_FILTER_LINEAR);
				rlTextureParameters(texture.id, RL_TEXTURE_MAG_FILTER, RL_FILTER_LINEAR);
			}
