...
void LoadFontDefault()
{
	...
	{
		debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] %s error in malloc chars\n",__FUNCTION__);
		return;
	}
	defaultFont.recs = (Rectangle *)malloc(defaultFont.charsCount*sizeof(Rectangle));
	if(defaultFont.recs==NULL)
	{
		debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] %s error in malloc recs\n",__FUNCTION__);
		return;
	}
	...
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s TEX ID=%i Default font loaded successfully\n",__FUNCTION__,defaultFont.texture.id);
}
...
void UnloadFontDefault(void)
{
	...
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s free chars\n",__FUNCTION__);
	free(defaultFont.recs);
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s free recs\n",__FUNCTION__);
}
...
Font LoadFont(const char *fileName)
{
	...
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s %s Font could not be loaded, using default font\n",__FUNCTION__,fileName);
		font = GetFontDefault();
	}
...
Font LoadFontEx(const char *fileName, int fontSize, int *fontChars, int charsCount)
{
	...
	if (font.chars != NULL)
	{
			debugNetPrintf(DEBUGNET_INFO,"fucking yeah!!!\n");
...
Font LoadFontFromImage(Image image, Color key, int firstChar)
{
	...
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] %s Font data parsed correctly from image\n",__FUNCTION__);
	// NOTE: We need to remove key color borders from image to avoid weird
	// artifacts on texture scaling when using FILTER_BILINEAR or FILTER_TRILINEAR
	for (int i = 0; i < image.height*image.width; i++) if (COLOR_EQUAL(pixels[i], key)) pixels[i] = BLANK;
	// Create a new image with the processed color data (key color replaced by BLANK)
	Image fontClear = LoadImageEx(pixels, image.width, image.height);
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s before free pixels\n",__FUNCTION__);
	free(pixels);    // Free pixels array memory
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s after free pixels\n",__FUNCTION__);
	...
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s Image file loaded correctly as Font\n",__FUNCTION__);
	return font;
}
...
CharInfo *LoadFontData(const char *fileName, int fontSize, int *fontChars, int charsCount, int type)
{
	...
	if (fileData != NULL)
	{
		...
		else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s %s Failed to process TTF font data",__FUNCTION__,fileName);
		free(fileData);
		if (genFontChars) free(fontChars);
	}
	else
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s %s TTF file could not be opened\n",__FUNCTION__,fileName);
	}
#else
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s %s TTF support is disabled\n",__FUNCTION__,fileName);
#endif
...
Image GenImageFontAtlas(const CharInfo *chars, Rectangle **charRecs, int charsCount, int fontSize, int padding, int packMethod)
{
	...
	*charRecs = NULL;
	...
	{
		debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] %s Using Skyline packing algorythm!\n",__FUNCTION__);
		...
		for (int i = 0; i < charsCount; i++)
		{
			...
			else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Character could not be packed: %i\n",__FUNCTION__,i);
		}
		free(rects);
		free(nodes);
		free(context);
...
void UnloadFont(Font font)
{
	...
	if (font.texture.id != GetFontDefault().texture.id)
	{
		...
		free(font.chars);
		free(font.recs);
		debugNetPrintf(DEBUGNET_INFO,"[ORBISGL]%s Unloaded sprite font data\n",__FUNCTION__);
	}
}
