...
#include <orbisGl2.h>
#include <orbisNfs.h>
extern PFNGLGENVERTEXARRAYSOESPROC glGenVertexArrays;        // Entry point pointer to function glGenVertexArrays()
extern PFNGLBINDVERTEXARRAYOESPROC glBindVertexArray;        // Entry point pointer to function glBindVertexArray()
extern PFNGLDELETEVERTEXARRAYSOESPROC glDeleteVertexArrays;  // Entry point pointer to function glDeleteVertexArrays()

extern rlglData RLGL;
void orbisGlPrintVertex(int drawCall,int offset, int count)
{
	int i=0;
	float *vertex=RLGL.State.vertexData[RLGL.State.currentBuffer].vertices+offset*3;//+3*RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter;
	while(i<count)//6*RLGL.State.draws[drawCall].vertexCount/4)
	{
		debugNetPrintf(3,"[ORBISGL][VERTEX] vcounter=%d drawCall=%d mode=%d vertexNumber=%d x=%f y=%f z=%f\n",RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter,drawCall,RLGL.State.draws[drawCall].mode,i,vertex[i*3],vertex[i*3+1],vertex[i*3+2]);
		i++;
	}
	sleep(1);
}
void orbisGlPrintTexCord(int drawCall, int offset,int count)
{
	int i=0;
	float *textCoord=RLGL.State.vertexData[RLGL.State.currentBuffer].texcoords+offset*2;//+2*RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter;
	while(i<count)//6*RLGL.State.draws[drawCall].vertexCount/4)
	{
		debugNetPrintf(3,"[ORBISGL][TEXTCOORD] drawCall=%d mode=%d textCordNumber=%d u=%f v=%f\n",drawCall,RLGL.State.draws[drawCall].mode,i,textCoord[i*2],textCoord[i+2+1]);
		i++;
	}
		sleep(1);

}
void orbisGlPrintColor(int drawCall,int offset,int count)
{
	int i=0;
	unsigned char *color=RLGL.State.vertexData[RLGL.State.currentBuffer].colors+offset*4;//+4*RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter;
	while(i<count)//6*RLGL.State.draws[drawCall].vertexCount/4)
	{
		debugNetPrintf(3,"[ORBISGL][TEXTCOORD] drawCall=%d mode=%d colorNumber=%d r=%d g=%d b=%d a=%d\n",drawCall,RLGL.State.draws[drawCall].mode,i,color[i*4],color[i*4+1],color[i*4+2],color[i*4+3]);
		i++;
	}
		sleep(1);

}
void orbisGlPrintDrawCounter(int i,int mode)
{
	switch(i)
	{
		case 0:	
			debugNetPrintf(3,"[ORBISGL] rlBegin init drawCounter=%d modeparam=%d mode=%d vertexCount=%d vcounter=%d\n",RLGL.State.drawsCounter,mode,RLGL.State.draws[RLGL.State.drawsCounter-1].mode,RLGL.State.draws[RLGL.State.drawsCounter-1].vertexCount,RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter);
			break;
		case 1:
			debugNetPrintf(3,"[ORBISGL] rlBegin end drawCounter=%d vertexCount=%d vcounter=%d\n",RLGL.State.drawsCounter,RLGL.State.draws[RLGL.State.drawsCounter-1].vertexCount,RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter);
			break;
		case 2: 			
			debugNetPrintf(3,"[ORBISGL] rlBegin middlea drawCounter=%d vertexCount=%d vcounter=%d\n",RLGL.State.drawsCounter,RLGL.State.draws[RLGL.State.drawsCounter-1].vertexCount,RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter);
			break;
		case 3:
			debugNetPrintf(3,"[ORBISGL] rlBegin middleb drawCounter=%d vertexCount=%d vcounter=%d\n",RLGL.State.drawsCounter,RLGL.State.draws[RLGL.State.drawsCounter-1].vertexCount,RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter);
			break;
		case 4:
			debugNetPrintf(3,"[ORBISGL] rlEnd  addColors=%d drawCounter=%d vertexCount=%d vcounter=%d\n",RLGL.State.drawsCounter,mode,RLGL.State.draws[RLGL.State.drawsCounter-1].vertexCount,RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter);
			break;
		case 5:
			debugNetPrintf(3,"[ORBISGL] rlEnd addTexCoords=%d drawCounter=%d vertexCount=%d vcounter=%d\n",RLGL.State.drawsCounter,mode,RLGL.State.draws[RLGL.State.drawsCounter-1].vertexCount,RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter);
			break;
		case 6:
			debugNetPrintf(3,"[ORBISGL] rlEnd vcount=%d drawCounter=%d vertexCount=%d vcounter=%d\n",RLGL.State.drawsCounter,mode,RLGL.State.draws[RLGL.State.drawsCounter-1].vertexCount,RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter);
			break;
	
	}
}
void orbisGlPrintDrawCall()
{
	int i;
	for(i=0;i<RLGL.State.drawsCounter;i++)
	{
		debugNetPrintf(3,"[ORBISGL] %d mode=%d vertexCount=%d vertexAlignment=%d textureId=%d\n",i,RLGL.State.draws[i].mode,RLGL.State.draws[i].vertexCount,RLGL.State.draws[i].vertexAlignment,RLGL.State.draws[i].textureId);
	}
}
...
{
	if (RLGL.State.stackCounter >= MAX_MATRIX_STACK_SIZE) debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Matrix RLGL.State.stack overflow\n",__FUNCTION__);
...
{
	//orbisGlPrintDrawCounter(0,mode);
    ...
    if (RLGL.State.draws[RLGL.State.drawsCounter - 1].mode != mode)
    {
        ...
        {
        	//orbisGlPrintDrawCounter(2,mode);
            ...
            //orbisGlPrintDrawCounter(3,mode);
    ...
    //orbisGlPrintDrawCounter(1,mode);
}
...
void rlEnd(void)
{
	...
	if (RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter != RLGL.State.vertexData[RLGL.State.currentBuffer].cCounter)
	{
		...
		//orbisGlPrintDrawCounter(4,addColors);
	...
	if (RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter != RLGL.State.vertexData[RLGL.State.currentBuffer].tcCounter)
	{
		...
		//orbisGlPrintDrawCounter(5,addTexCoords);
	...
	{
		//orbisGlPrintDrawCounter(6,RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter);
...
void rlVertex3f(float x, float y, float z)
{
	...
	if (RLGL.State.doTransform!=0) vec = Vector3Transform(vec, RLGL.State.transform);
	...
	else debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] %s MAX_BATCH_ELEMENTS overflow\n",__FUNCTION__);
}
	...
	switch (param)
	{
		...
		case RL_TEXTURE_WRAP_T:
		{
			if (value == RL_WRAP_MIRROR_CLAMP)
			{
				...
				else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Clamp mirror wrap mode not supported\n",__FUNCTION__);
			}
		...
		case RL_TEXTURE_ANISOTROPIC_FILTER:
		{
			...
			{
				debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [TEX ID %i] Maximum anisotropic filter level supported is %iX\n",__FUNCTION__,id, RLGL.ExtSupported.maxAnisotropicLevel);
				glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, (float)value);
			}
			else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Anisotropic filtering not supported\n",__FUNCTION__);
		} break;
		default: break;
	}
...
void rlDeleteRenderTextures(RenderTexture2D target)
{
	...
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s [FBO ID %i] Unloaded render texture data from VRAM (GPU)\n",__FUNCTION__, target.id);
}
...
void rlDeleteVertexArrays(unsigned int id)
{
	if (RLGL.ExtSupported.vao)
	{
		...
		debugNetPrintf(DEBUGNET_INFO, "[ORBISG] %s [VAO ID %i] Unloaded model data from VRAM (GPU)\n",__FUNCTION__, id);
	}
}
...
void rlDeleteBuffers(unsigned int id)
{
	if (id != 0)
	{
		...
		if (!RLGL.ExtSupported.vao) debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s [VBO ID %i] Unloaded model vertex data from VRAM (GPU)\n",__FUNCTION__,id);
	}
}
...
void rlglInit(int width, int height)
{
	...
	debugNetPrintf(DEBUGNET_INFO, "GPU: Vendor:   %s", glGetString(GL_VENDOR));
	debugNetPrintf(DEBUGNET_INFO, "GPU: Renderer: %s", glGetString(GL_RENDERER));
	debugNetPrintf(DEBUGNET_INFO, "GPU: Version:  %s", glGetString(GL_VERSION));
	debugNetPrintf(DEBUGNET_INFO, "GPU: GLSL:     %s", glGetString(GL_SHADING_LANGUAGE_VERSION));
	...
	debugNetPrintf(DEBUGNET_INFO, "Number of supported extensions: %i", numExt);
	...
	if (RLGL.ExtSupported.vao) debugNetPrintf(DEBUGNET_INFO, "[EXTENSION] VAO extension detected, VAO functions initialized successfully");
	else debugNetPrintf(DEBUGNET_ERROR, "[EXTENSION] VAO extension not found, VAO usage not supported");
	if (RLGL.ExtSupported.texNPOT) debugNetPrintf(DEBUGNET_INFO, "[EXTENSION] NPOT textures extension detected, full NPOT textures supported");
	else debugNetPrintf(DEBUGNET_ERROR, "[EXTENSION] NPOT textures extension not found, limited NPOT support (no-mipmaps, no-repeat)");
	if (RLGL.ExtSupported.texCompDXT) debugNetPrintf(DEBUGNET_INFO, "[EXTENSION] DXT compressed textures supported");
	if (RLGL.ExtSupported.texCompETC1) debugNetPrintf(DEBUGNET_INFO, "[EXTENSION] ETC1 compressed textures supported");
	if (RLGL.ExtSupported.texCompETC2) debugNetPrintf(DEBUGNET_INFO, "[EXTENSION] ETC2/EAC compressed textures supported");
	if (RLGL.ExtSupported.texCompPVRT) debugNetPrintf(DEBUGNET_INFO, "[EXTENSION] PVRT compressed textures supported");
	if (RLGL.ExtSupported.texCompASTC) debugNetPrintf(DEBUGNET_INFO, "[EXTENSION] ASTC compressed textures supported");
	if (RLGL.ExtSupported.texAnisoFilter) debugNetPrintf(DEBUGNET_INFO, "[EXTENSION] Anisotropic textures filtering supported (max: %.0fX)", RLGL.ExtSupported.maxAnisotropicLevel);
	if (RLGL.ExtSupported.texMirrorClamp) debugNetPrintf(DEBUGNET_INFO, "[EXTENSION] Mirror clamp wrap texture mode supported");
	if (RLGL.ExtSupported.debugMarker) debugNetPrintf(DEBUGNET_INFO, "[EXTENSION] Debug Marker supported");
	// Initialize buffers, default shaders and default textures
	//----------------------------------------------------------
	// Init default white texture
	unsigned char pixels[4] = { 255, 255, 255, 255 };   // 1 pixel RGBA (4 bytes)
	RLGL.State.defaultTextureId = rlLoadTexture(pixels, 1, 1, UNCOMPRESSED_R8G8B8A8, 1);
	if (RLGL.State.defaultTextureId != 0) debugNetPrintf(DEBUGNET_INFO, "[TEX ID %i] Base white texture loaded successfully", RLGL.State.defaultTextureId);
	else debugNetPrintf(DEBUGNET_ERROR, "Base white texture could not be loaded");
	...
	debugNetPrintf(DEBUGNET_INFO, "OpenGL default states initialized successfully");
}
...
void rlglClose(void)
{
	...
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] [TEX ID %i] Unloaded texture data (base white texture) from VRAM\n", RLGL.State.defaultTextureId);
...
unsigned int rlLoadTexture(void *data, int width, int height, int format, int mipmapCount)
{
	...
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s DXT compressed texture format not supported\n",__FUNCTION__);
		return id;
	}
	if ((!RLGL.ExtSupported.texCompETC1) && (format == COMPRESSED_ETC1_RGB))
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s ETC1 compressed texture format not supported\n",__FUNCTION__);
		return id;
	}
	if ((!RLGL.ExtSupported.texCompETC2) && ((format == COMPRESSED_ETC2_RGB) || (format == COMPRESSED_ETC2_EAC_RGBA)))
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s ETC2 compressed texture format not supported\n",__FUNCTION__);
		return id;
	}
	if ((!RLGL.ExtSupported.texCompPVRT) && ((format == COMPRESSED_PVRT_RGB) || (format == COMPRESSED_PVRT_RGBA)))
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s PVRT compressed texture format not supported\n",__FUNCTION__);
		return id;
	}
	if ((!RLGL.ExtSupported.texCompASTC) && ((format == COMPRESSED_ASTC_4x4_RGBA) || (format == COMPRESSED_ASTC_8x8_RGBA)))
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s ASTC compressed texture format not supported\n",__FUNCTION__);
		return id;
	}
	...
	debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s Load texture from data memory address: 0x%x\n",__FUNCTION__,data);
	...
	for (int i = 0; i < mipmapCount; i++)
	{
		...
		debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s Load mipmap level %i (%i x %i), size: %i, offset: %i\n",__FUNCTION__,i, mipWidth, mipHeight, mipSize, mipOffset);
	...
	glBindTexture(GL_TEXTURE_2D, 0);
	if (id > 0) debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s [TEX ID %i] Texture created successfully (%ix%i - %i mipmaps)\n",__FUNCTION__,id, width, height, mipmapCount);
	else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Texture could not be created\n",__FUNCTION__);
	return id;
}
...
void rlUpdateTexture(unsigned int id, int width, int height, int format, const void *data)
{
	...
	else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Texture format updating not supported\n",__FUNCTION__);
}
...
void rlGetGlTextureFormats(int format, unsigned int *glInternalFormat, unsigned int *glFormat, unsigned int *glType)
{
	...
	*glType = -1;
	switch (format)
	{
		...
		default: debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Texture format not supported\n",__FUNCTION__); break;
	}
}
...
RenderTexture2D rlLoadRenderTexture(int width, int height, int format, int depthBits, bool useDepthTexture)
{
	...
	//-----------------------------------------------------------------------------------------------------
	if (rlRenderTextureComplete(target)) debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s [FBO ID %i] Framebuffer object created successfully\n",__FUNCTION__,target.id);
	//-----------------------------------------------------------------------------------------------------
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	return target;
}
...
bool rlRenderTextureComplete(RenderTexture target)
{
	...
	if (status != GL_FRAMEBUFFER_COMPLETE)
	{
		switch (status)
		{
			case GL_FRAMEBUFFER_UNSUPPORTED: debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Framebuffer is unsupported\n",__FUNCTION__); break;
			case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT: debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Framebuffer has incomplete attachment\n",__FUNCTION__); break;
			case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS: debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Framebuffer has incomplete dimensions\n",__FUNCTION__); break;
			case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT: debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Framebuffer has a missing attachment\n",__FUNCTION__); break;
			default: break;
		}
	}
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
...
void rlGenerateMipmaps(Texture2D *texture)
{
	...
	if ((texIsPOT) || (RLGL.ExtSupported.texNPOT))
	{
		...
		debugNetPrintf(DEBUGNET_INFO, "[ORBIS] %s [TEX ID %i] Mipmaps generated automatically\n",__FUNCTION__,texture->id);
	...
	else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [TEX ID %i] Mipmaps can not be generated\n",__FUNCTION__,texture->id);
	glBindTexture(GL_TEXTURE_2D, 0);
}
...
void rlLoadMesh(Mesh *mesh, bool dynamic)
{
	if (mesh->vaoId > 0)
	{
		...
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Trying to re-load an already loaded mesh\n",__FUNCTION__);
		return;
	}
	...
	if (RLGL.ExtSupported.vao)
	{
		if (mesh->vaoId > 0) debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s [VAO ID %i] Mesh uploaded successfully to VRAM (GPU)\n",__FUNCTION__,mesh->vaoId);
		else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Mesh could not be uploaded to VRAM (GPU)\n",__FUNCTION__);
	}
	else
	{
		debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s [VBOs] Mesh uploaded successfully to VRAM (GPU)\n",__FUNCTION__);
	}
}
...
Shader LoadShader(const char *vsFileName, const char *fsFileName)
{
	...
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] %s 1\n",__FUNCTION__);

	shader = LoadShaderCode(vShaderStr, fShaderStr);
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] %s 2\n",__FUNCTION__);
...
Shader LoadShaderCode(const char *vsCode, const char *fsCode)
{
	...
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] %s vid=%d fid=%d\n",__FUNCTION__,vertexShaderId,fragmentShaderId);
	...
	else
	{
		...
		if (shader.id == 0)
		{
			debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Custom shader could not be loaded\n",__FUNCTION__);
	...
	for (int i = 0; i < uniformCount; i++)
	{
		...
		name[namelen] = 0;
		debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] %s [SHDR ID %i] Active uniform [%s] set at location: %i\n",__FUNCTION__, shader.id, name, glGetUniformLocation(shader.id, name));
	}
	return shader;
}
...
void UnloadShader(Shader shader)
{
	if (shader.id > 0)
	{
		...
		debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s [SHDR ID %i] Unloaded shader program data\n",__FUNCTION__,shader.id);
	}
	free(shader.locs);
}
...
int GetShaderLocation(Shader shader, const char *uniformName)
{
	...
	if (location == -1) debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [SHDR ID %i][%s] Shader uniform could not be found\n",__FUNCTION__, shader.id, uniformName);
	else debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s [SHDR ID %i][%s] Shader uniform set at location: %i\n",__FUNCTION__,shader.id, uniformName, location);
	return location;
}
...
void SetShaderValueV(Shader shader, int uniformLoc, const void *value, int uniformType, int count)
{
	...
	switch (uniformType)
	{
		...
		default: debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Shader uniform could not be set data type not recognized\n",__FUNCTION__);
	}
...
unsigned int CompileShader(const char *shaderStr, int type)
{
	...
	if (success != GL_TRUE)
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [SHDR ID %i] Failed to compile shader...\n",__FUNCTION__,shader);
		int maxLength = 0;
		int length;
		glGetShaderiv(shader, GL_INFO_LOG_LENGTH, &maxLength);
		char log[maxLength];
		glGetShaderInfoLog(shader, maxLength, &length, log);
		debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s %s\n",__FUNCTION__,log);
	}
	else debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s [SHDR ID %i] Shader compiled successfully\n",__FUNCTION__,shader);
	return shader;
}
...
unsigned int LoadShaderProgram(unsigned int vShaderId, unsigned int fShaderId)
{
	...
	if (success == GL_FALSE)
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [SHDR ID %i] Failed to link shader program...\n",__FUNCTION__,program);
		int maxLength = 0;
		int length;
	...
	else debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s [SHDR ID %i] Shader program loaded successfully\n",__FUNCTION__,program);
	return program;
}
...
Shader LoadShaderDefault(void)
{
	...
	if (shader.id > 0)
	{
		debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s [SHDR ID %i] Default shader loaded successfully\n",__FUNCTION__,shader.id);
	...
	else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [SHDR ID %i] Default shader could not be loaded\n",__FUNCTION__,shader.id);
...
void LoadBuffersDefault(void)
{
	...
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s Internal buffers initialized successfully (CPU)\n",__FUNCTION__);
	//--------------------------------------------------------------------------------------------
	...
	for (int i = 0; i < MAX_BATCH_BUFFERING; i++)
	{
		...
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned short)*6*MAX_BATCH_ELEMENTS, RLGL.State.vertexData[i].indices, GL_STATIC_DRAW);
	}
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s Internal buffers uploaded successfully (GPU)\n",__FUNCTION__);
	// Unbind the current VAO
	//if (RLGL.ExtSupported.vao) glBindVertexArray(0);
	//--------------------------------------------------------------------------------------------
}
...
void DrawBuffersDefault(void)
{
    ...
    for (int eye = 0; eye < eyesCount; eye++)
    {
        ...
        if (RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter > 0)
        {
            ...
            if (RLGL.ExtSupported.vao) 
            {//glBindVertexArray(RLGL.State.vertexData[RLGL.State.currentBuffer].vaoId);
            }
            else
            {
            ...
            glActiveTexture(GL_TEXTURE0);
            //orbisGlPrintDrawCall();
            for (int i = 0; i < RLGL.State.drawsCounter; i++)
            {
            	//orbisGlPrintVertex(i,vertexOffset,RLGL.State.draws[i].vertexCount);
            	//orbisGlPrintTexCord(i,vertexOffset,RLGL.State.draws[i].vertexCount);
            	//orbisGlPrintColor(i,vertexOffset,RLGL.State.draws[i].vertexCount);
                ...
                if ((RLGL.State.draws[i].mode == RL_LINES) || (RLGL.State.draws[i].mode == RL_TRIANGLES)) {glDrawArrays(RLGL.State.draws[i].mode, vertexOffset, RLGL.State.draws[i].vertexCount); //int ret=glGetError();
		//	debugNetPrintf(3,"[ORBISGL] glDrawArrays return: 0x%08X vertexOffset=%d vertexCount=%d vertexAlignment=%d\n",ret,vertexOffset,RLGL.State.draws[i].vertexCount,RLGL.State.draws[i].vertexAlignment);
                }
                else
                {
                    ...
                    glDrawElements(GL_TRIANGLES, RLGL.State.draws[i].vertexCount/4*6, GL_UNSIGNED_SHORT, (GLvoid *)(sizeof(GLushort)*vertexOffset/4*6));
               		//int ret1=glGetError();
                	//debugNetPrintf(3,"[ORBISGL] glDrawElements return: 0x%08X count=%d  index=%d vertexoffset=%d vertexCount=%d vertexAlignment=%d\n",ret1,RLGL.State.draws[i].vertexCount/4*6,(sizeof(GLushort)*vertexOffset/4*6),vertexOffset,RLGL.State.draws[i].vertexCount,RLGL.State.draws[i].vertexAlignment);
