...
PFNGLGENVERTEXARRAYSOESPROC glGenVertexArrays;        // Entry point pointer to function glGenVertexArrays()
PFNGLBINDVERTEXARRAYOESPROC glBindVertexArray;        // Entry point pointer to function glBindVertexArray()
PFNGLDELETEVERTEXARRAYSOESPROC glDeleteVertexArrays;  // Entry point pointer to function glDeleteVertexArrays()
...
void EndDrawing(void)
{
	if(orbisGlConf)
	{
		//DrawRectangle(orbisGlConf->position.x, orbisGlConf->position.y, 3, 3, MAROON);
...
void SetTargetFPS(int fps)
{
	...
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s Target time per frame: %02.03f milliseconds\n",__FUNCTION__,(float)orbisGlConf->target*1000);
}
...
double GetTime(void)
{
	//debugNetPrintf(DEBUGNET_INFO, "[DEBUG] %s \n",__FUNCTION__);

    struct timespec ts;
    clock_gettime(CLOCK_MONOTONIC, &ts);
    //debugNetPrintf(DEBUGNET_INFO, "[DEBUG] %s after clock_gettime\n",__FUNCTION__);
...
void SetupFramebuffer(int width, int height)
{
	...
	if ((orbisGlConf->screen.width > orbisGlConf->display.width) || (orbisGlConf->screen.height > orbisGlConf->display.height))
	{
		...
		orbisGlConf->render.width = orbisGlConf->display.width;
		orbisGlConf->render.height = orbisGlConf->display.height;
		debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s Downscale matrix generated, content will be rendered at: %i x %i\n",__FUNCTION__,orbisGlConf->render.width, orbisGlConf->render.height);
	}
	else if ((orbisGlConf->screen.width < orbisGlConf->display.width) || (orbisGlConf->screen.height < orbisGlConf->display.height))
	{
		// Required screen size is smaller than display size
		debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s UPSCALING: Required screen size: %i x %i -> Display size: %i x %i\n",__FUNCTION__,orbisGlConf->screen.width, orbisGlConf->screen.height, orbisGlConf->display.width, orbisGlConf->display.height);
...
void InitTimer(void)
{
	debugNetPrintf(DEBUGNET_INFO, "[DEBUG] %s before srand\n",__FUNCTION__);
    srand((unsigned int)time(NULL));              // Initialize random seed

	debugNetPrintf(DEBUGNET_INFO, "[DEBUG] %s after srand\n",__FUNCTION__);

    struct timespec now;

    if (clock_gettime(CLOCK_MONOTONIC, &now) == 0)  // Success
    {
    	debugNetPrintf(DEBUGNET_INFO, "[DEBUG] %s in if \n",__FUNCTION__);
        orbisGlConf->base = (unsigned long long int)now.tv_sec*1000000000LLU + (unsigned long long int)now.tv_nsec;
    }
    else debugNetPrintf(DEBUGNET_INFO, "[DEBUG] %s No hi-resolution timer available\n",__FUNCTION__);
	debugNetPrintf(DEBUGNET_INFO, "[DEBUG] %s before GetTime\n",__FUNCTION__);
    orbisGlConf->previous = GetTime();       // Get time as double
    debugNetPrintf(DEBUGNET_INFO, "[DEBUG] %s after GetTime\n",__FUNCTION__);
...
int orbisGlCreateConf(unsigned int width,unsigned int height)
{
	if(!orbisGlConf)
	{
		...
		memset(&orbisGlConf->pgl_config, 0, sizeof(orbisGlConf->pgl_config));
		{
			...
			orbisGlConf->pgl_config.systemSharedMemorySize=250*1024*1024;
			orbisGlConf->pgl_config.videoSharedMemorySize=512*1024*1024; 
			orbisGlConf->pgl_config.maxMappedFlexibleMemory=170*1024*1024;
			orbisGlConf->pgl_config.drawCommandBufferSize=1*1024*1024;
			orbisGlConf->pgl_config.lcueResourceBufferSize=1*1024*1024;
...
void InitWindow(int width, int height, const char *title)
{
    ...
    {
    	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] orbisGlInit return error\n");
    }
}
void CloseWindow()
{
    ...
    if (fileName != NULL)
    {
        ...
        if (textFile>=0)
        {
            ...
            if (size > 0)
            {
                ...
                debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s FILEIO: [%s] Text file loaded successfully\n",__FUNCTION__,fileName);
            }
            else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s FILEIO: [%s] Failed to read text file\n",__FUNCTION__,fileName);

            orbisNfsClose(textFile);
        }
        else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s FILEIO: [%s] Failed to open text file\n",__FUNCTION__,fileName);
    }
    else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s FILEIO: File name provided is not valid\n",__FUNCTION__);
...
// NOTE: Duplicated string (extensionsDup) must be deallocated
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] Number of supported extensions: %i\n", numExt);
	...
	if (RLGL.ExtSupported.vao) debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] VAO extension detected, VAO functions initialized successfully\n");
	else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] VAO extension not found, VAO usage not supported\n");
	if (RLGL.ExtSupported.texNPOT) debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] NPOT textures extension detected, full NPOT textures supported\n");
	else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] NPOT textures extension not found, limited NPOT support (no-mipmaps, no-repeat)\n");
	if (RLGL.ExtSupported.texCompDXT) debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] DXT compressed textures supported\n");
	if (RLGL.ExtSupported.texCompETC1) debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] ETC1 compressed textures supported\n");
	if (RLGL.ExtSupported.texCompETC2) debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] ETC2/EAC compressed textures supported\n");
	if (RLGL.ExtSupported.texCompPVRT) debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] PVRT compressed textures supported\n");
	if (RLGL.ExtSupported.texCompASTC) debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] ASTC compressed textures supported\n");
	if (RLGL.ExtSupported.texAnisoFilter) debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] Anisotropic textures filtering supported (max: %.0fX)\n", RLGL.ExtSupported.maxAnisotropicLevel);
	if (RLGL.ExtSupported.texMirrorClamp) debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] Mirror clamp wrap texture mode supported\n");
	if (RLGL.ExtSupported.debugMarker) debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] Debug Marker supported\n");
	// Initialize buffers, default shaders and default textures
	//----------------------------------------------------------
	// Init default white texture
	unsigned char pixels[4] = { 255, 255, 255, 255 };   // 1 pixel RGBA (4 bytes)
	RLGL.State.defaultTextureId = rlLoadTexture(pixels, 1, 1, UNCOMPRESSED_R8G8B8A8, 1);
	if (RLGL.State.defaultTextureId != 0) debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] [TEX ID %i] Base white texture loaded successfully\n", RLGL.State.defaultTextureId);
	else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] Base white texture could not be loaded\n");
	...
	debugNetPrintf(3, "[ORBISGL] doTransform=%d\n",RLGL.State.doTransform);
	RLGL.State.doTransform=false;
	debugNetPrintf(3, "[ORBISGL] doTransform=%d\n",RLGL.State.doTransform);
	RLGL.State.doTransform=0;
	debugNetPrintf(3, "[ORBISGL] doTransform=%d\n",RLGL.State.doTransform);
	...
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] default states initialized successfully\n");
	...
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] after InitTimer\n");


	orbisGlConf->orbisgl_initialized=1;
	ClearBackground(RAYWHITE);
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] after ClearBackground\n");

	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] liborbisGl2 initialized\n");
	return orbisGlConf->orbisgl_initialized;
}
