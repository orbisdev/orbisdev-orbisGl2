...
Model LoadModel(const char *fileName)
{
...
#if defined(SUPPORT_MESH_GENERATION)
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [%s] No meshes can be loaded, default to cube mesh\n",__FUNCTION__,fileName);
		model.meshes[0] = GenMeshCube(1.0f, 1.0f, 1.0f);
#else
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [%s] No meshes can be loaded, and can't create a default mesh. The raylib mesh generation is not supported (SUPPORT_MESH_GENERATION).\n",__FUNCTION__ ,fileName);
#endif
	}
	else
	{
	...
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [%s] No materials can be loaded, default to white material\n",__FUNCTION__ ,fileName);
...
void UnloadModel(Model model)
{
	...
	free(model.bones);
	free(model.bindPose);
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s Unloaded model data from RAM and VRAM\n",__FUNCTION__);
}
...
Material *LoadMaterials(const char *fileName, int *materialCount)
{
	...
	if (IsFileExtension(fileName, ".mtl"))
	{
		...
		if (result != TINYOBJ_SUCCESS) {
			debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [%s] Could not parse Materials file\n",__FUNCTION__,fileName);
		}
		// TODO: Process materials to return
		tinyobj_materials_free(mats, count);
	}
#else
	debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [%s] Materials file not supported",__FUNCTION__,fileName);
#endif
...
{
	if (meshId >= model->meshCount) debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Mesh id greater than mesh count\n",__FUNCTION__);
	else if (materialId >= model->materialCount) debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] %s Material id greater than material count\n",__FUNCTION__);
...
ModelAnimation *LoadModelAnimations(const char *filename, int *animCount)
{
	...
	if (iqmFile<0)
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [%s] Unable to open file\n",__FUNCTION__,filename);
		return NULL;
	}
	// Read IQM header
	...
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Magic Number \"%s\"does not match.\n",__FUNCTION__,iqm.magic);
		orbisNfsClose(iqmFile);
		return NULL;
	}
	if (iqm.version != IQM_VERSION)
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s IQM version %i is incorrect.\n",__FUNCTION__,iqm.version);
		orbisNfsClose(iqmFile);
		return NULL;
	}
	// Get bones data
...
void MeshTangents(Mesh *mesh)
{
	...
	else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s Mesh tangents already exist\n",__FUNCTION__);
	...
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s Tangents computed for mesh\n",__FUNCTION__);
}
...
static Model LoadOBJ(const char *fileName)
{
	...
	if (data != NULL)
	{
		...
		if (ret != TINYOBJ_SUCCESS) debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [%s] Model data could not be loaded\n",__FUNCTION__,fileName);
		else debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s [%s] Model data loaded successfully: %i meshes / %i materials\n",__FUNCTION__,fileName, meshCount, materialCount);
	...
	debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s [%s] Model loaded successfully in RAM (CPU)\n",__FUNCTION__,fileName);
	return model;
}
#endif
...
static Model LoadIQM(const char *fileName)
{
	...
	*/
	...
	if (iqmFile < 0)
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [%s] IQM file could not be opened\n",__FUNCTION__,fileName);
		return model;
	}
	orbisNfsRead(iqmFile,&iqm,sizeof(IQMHeader));  // Read IQM header
	if (strncmp(iqm.magic, IQM_MAGIC, sizeof(IQM_MAGIC)))
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [%s] IQM file does not seem to be valid\n",__FUNCTION__,fileName);
		orbisNfsClose(iqmFile);
		return model;
	}
	if (iqm.version != IQM_VERSION)
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [%s] IQM file version is not supported (%i).\n",__FUNCTION__,fileName, iqm.version);
		orbisNfsClose(iqmFile);
		return model;
	}
...
static Image LoadImageFromCgltfImage(cgltf_image *image, const char *texPath, Color tint)
{
    ...
    if (image->uri)
    {
            ...
            (image->uri[4] == ':'))
        {
            ...
            if (image->uri[i] == 0) debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s CGLTF Image: Invalid data URI\n",__FUNCTION__);
            else
            {
                int size;
...
static Model LoadGLTF(const char *fileName)
{
	...
	*************************************************************************************/
	...
	if (gltfFile < 0)
	{
		debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [%s] glTF file could not be opened\n",__FUNCTION__,fileName);
		return model;
	}
	...
	{
		debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s [%s][%s] Model meshes/materials: %i/%i\n",__FUNCTION__,fileName, (data->file_type == 2)? "glb" : "gltf", data->meshes_count, data->materials_count);
		// Read data buffers
		result = cgltf_load_buffers(&options, data, fileName);
		if (result != cgltf_result_success) debugNetPrintf(DEBUGNET_INFO, "[ORBISGL] %s [%s][%s] Error loading mesh/material buffers\n",__FUNCTION__,fileName, (data->file_type == 2)? "glb" : "gltf");
			...
			for (int p = 0; p < data->meshes[i].primitives_count; p++)
			{
				for (int j = 0; j < data->meshes[i].primitives[p].attributes_count; j++)
				{
					...
					else if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_texcoord)
					{
						...
						else
						{
							...
							debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [%s] Texture coordinates must be float\n",__FUNCTION__,fileName);
						}
					}
				}
				...
				if (acc)
				{
					...
					else
					{
						...
						debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [%s] Indices must be unsigned short\n",__FUNCTION__,fileName);
					}
				}
				else
				{
	...
	else debugNetPrintf(DEBUGNET_ERROR, "[ORBISGL] %s [%s] glTF data could not be loaded\n",__FUNCTION__,fileName);
	free(buffer);
	return model;
}
#endif
