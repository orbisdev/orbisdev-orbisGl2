...
void DrawCircleLines(int centerX, int centerY, float radius, Color color)
{
	...
	for (int i = 0; i < 360; i += 10)
	{
		//debugNetPrintf(3,"[ORBISGL] %s %d currentDepth=%d doTransform=%d\n",__FUNCTION__,i,RLGL.State.currentDepth,RLGL.State.doTransform);
		//debugNetPrintf(3,"[ORBISGL] %s centerX=%d sinf=%f deg2=%f radius=%f\n",__FUNCTION__,centerX,sinf(DEG2RAD*i)*radius,DEG2RAD,radius);
		//debugNetPrintf(3,"[ORBISGL] %s %f %f \n",__FUNCTION__,centerX + sinf(DEG2RAD*i)*radius, centerY + cosf(DEG2RAD*i)*radius);
		rlVertex2f(centerX + sinf(DEG2RAD*i)*radius, centerY + cosf(DEG2RAD*i)*radius);
		rlVertex2f(centerX + sinf(DEG2RAD*(i + 10))*radius, centerY + cosf(DEG2RAD*(i + 10))*radius);
	}
	rlEnd();
